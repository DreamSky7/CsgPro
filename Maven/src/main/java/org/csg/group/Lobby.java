...
public class Lobby implements CycleUpdate, LobbyAPI {
	...
	public VarTable macros = new VarTable(this);
	...
	public void load(){
		...
		functions.clear();
		listener.clear();
		MessageUtils.ConsoleInfoMsg("===== [ 正在加载大厅 &2" + Name + " &r] =====");

		try{
			loadFile(Folder);
		}catch(IOException e){
			e.printStackTrace();
			MessageUtils.ConsoleInfoMsg("文件读取失败！");
		}

		addMacro("world", worldName);
		...
		if(isComplete()){
			...
			MessageUtils.ConsoleInfoMsg("===== [ 大厅 "+Name+" &a加载成功 &r| =====");
			SecondCycle.registerCall(this);

		}else{
			MessageUtils.ConsoleInfoMsg("===== [ 大厅 "+Name+" &c加载失败 &r| =====");
		}
	}
	...
	public void unLoad() {
		...
		if (isSproom) {
			Fwmain.temporaryWorldManager.unloadTemporaryWorld(worldName);
		}
		...
		MessageUtils.ConsoleInfoMsg("===== [ 大厅 "+Name+" &a卸载成功 &r| =====");
	}
	...
	public boolean requireMacro(String line){
		...
		if(state!=2){
			MessageUtils.ConsoleErrorMsg("该大厅并未满足脚本需求的宏"+s+"！");
			MessageUtils.ConsoleErrorMsg("请在macro.yml文件设置相关内容，并重载该大厅。在此之前，相关脚本将无法使用！");
		}
		if(state==0){
			try{
	...
	public Object callFunction(String name, Player p, Object... para){
		...
		if(javaFunctionClass !=null){
			try{
				...
				e.printStackTrace();
				MessageUtils.ConsoleInfoMsg("尝试调取java函数"+name+"失败！");
			}
		}
		return null;
	}
	...
	public void callListener(String name, Player p, Object... para){
		...
		if(javaFunctionClass !=null){
			try{
				...
				e.printStackTrace();
				MessageUtils.ConsoleInfoMsg("尝试调取java监听器"+name+"失败！");
			}
		}
	}
	...
	private void loadFile(File folder) throws IOException {
		...
		if(default_macro_file!=null){
			//MessageUtils.ConsoleInfoMsg("正在加载Macro预设宏");
		...
		if (macros.macros.containsKey("urlResource")) {
			String urlResource = (String) macros.macros.get("urlResource");
			String urlKey = null;

			// 判断是否有密钥
			if (macros.macros.containsKey("urlKey")) {
				urlKey = (String) macros.macros.get("urlKey");
			}

			// 验证url是否合法
			if (jcompiler.isValidUrl(urlResource)) {
				Map<String, String> params = new HashMap<>();

				if (StringUtils.isNotBlank(urlKey)) {
					params.put("key", urlKey); // 设置请求参数
				}

				jcompiler.loadJavaTaskFromUrl(urlResource, params, inputStream -> {
					if (inputStream != null) {
						try {
							jcompiler.read(inputStream);  // 使用jcompiler的新方法处理输入流
						} catch (IOException e) {
							e.printStackTrace();
						}
					}
				...
				MessageUtils.ConsoleErrorMsg("&c请填写正确的url地址！");
			}
		}
