...
public class TeamManager {
    ...
    public static void addTeam(String teamName) {
        ...
        if (scoreboard.getTeam(teamName) != null) {
            MessageUtils.ConsoleWarnMsg("该队伍已经存在！请检查是否重复添加！");
        }
    ...
    public static void delTeam(String teamName) {
        // 判断是否已经存在这个队伍
        if (scoreboard.getTeam(teamName) == null) {
            MessageUtils.ConsoleWarnMsg("该队伍不存在！请检查是否重复删除！");
        }
    ...
    public static void joinTeam(String teamName, String playerName) {
        ...
        if (scoreboard.getEntryTeam(playerName) != null) {
            MessageUtils.ConsoleWarnMsg("该玩家已经存在！请检查是否重复添加！");
        }
    ...
    public static void leaveTeam(String teamName, String playerName) {
        // 判断是否已经存在这个队伍
        if (scoreboard.getTeam(teamName) == null) {
            MessageUtils.ConsoleWarnMsg("该队伍不存在！请先添加队伍！");
        }
        // 判断是否已经存在这个玩家
        if (scoreboard.getEntryTeam(playerName) == null) {
            MessageUtils.ConsoleWarnMsg("该玩家不存在！请检查是否重复删除！");
        }
            ...
            case "nametag":
                switch (value) {
                    ...
                    case "3":
                        ...
                        break;
                    default:
                        MessageUtils.ConsoleWarnMsg("无效的选项！");
                }
                break;
            default:
                MessageUtils.ConsoleWarnMsg("无效的选项！");
    ...
    public static void infoTeam(String teamName, Player targer) {
        ...
        if (targer != null) {
            ...
            MessageUtils.ConsoleInfoMsg("§7队伍名称：§a" + teamName);
            MessageUtils.ConsoleInfoMsg("§7队伍人数：§a" + scoreboard.getTeam(teamName).getSize());
            MessageUtils.ConsoleInfoMsg("§7队伍前缀：§a" + scoreboard.getTeam(teamName).getPrefix());
            MessageUtils.ConsoleInfoMsg("§7队伍后缀：§a" + scoreboard.getTeam(teamName).getSuffix());
            MessageUtils.ConsoleInfoMsg("§7队伍PVP：§a" + scoreboard.getTeam(teamName).allowFriendlyFire());
            MessageUtils.ConsoleInfoMsg("§7名称显示：§a" + scoreboard.getTeam(teamName).getOption(Team.Option.NAME_TAG_VISIBILITY));
        }
    }
