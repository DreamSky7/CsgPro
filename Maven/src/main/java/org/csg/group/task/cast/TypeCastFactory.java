    ...
    public static Object castObject(Object object, Type target){
        ...
        if(target instanceof Class<?>){
            ...
            if(!castAvailable((Class<?>)target)){
                MessageUtils.ConsoleErrorMsg("csg暂时不支持"+((Class<?>)target).getName()+"的类型转化！");
                return null;
            }
        }
        if(target instanceof ParameterizedType){
            ...
            if(!castAvailable((Class<?>)p.getRawType())){
                MessageUtils.ConsoleErrorMsg("csg暂时不支持"+((Class<?>)p.getRawType()).getName()+"的类型转化！");
                return null;
            }
        }

        if(!castAvailable(object.getClass())){
            MessageUtils.ConsoleErrorMsg("csg暂时不支持"+object.getClass().getName()+"的类型转化！");
            return null;
        }
