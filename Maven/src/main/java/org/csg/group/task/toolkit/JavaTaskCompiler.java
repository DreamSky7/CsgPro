...
public class JavaTaskCompiler extends ClassLoader {
    ...
    public JavaTaskCompiler(Lobby lobby) {
        ...
        imports.add("org.csg.group.Lobby;");
     ...
     * @param callback
     */
    public void loadJavaTaskFromUrl(String urlString, Map<String, String> params, Consumer<InputStream> callback) {
        Bukkit.getScheduler().runTaskAsynchronously(Fwmain.getInstance(), () -> {
            try {
    ...
    public void addDepend(File file) {
        ...
        if (s.contains(".")) {
            MessageUtils.ConsoleInfoMsg("正在添加java脚本依赖" + s);
    ...
    public boolean isValidUrl(String urlString) {
        ...
        return Pattern.matches(regex, urlString);
    }

    public void read(InputStream inputStream) throws IOException {
        hasContent = true;
        MessageUtils.ConsoleInfoMsg("正在加载URL网络JavaTask");
            ...
            switch (cm[0]) {
                ...
                case "depend":
                    if (!Fwmain.getInstance().getServer().getPluginManager().isPluginEnabled(cm[1])) {
                        MessageUtils.ConsoleInfoMsg("该大厅并未满足脚本需求的插件依赖" + cm[1] + "！");
                        MessageUtils.ConsoleInfoMsg("请添加所需的前置插件，并重启服务器。在此之前，相关脚本将无法使用！");
                        pass = false;
                    }
                    break;
                case "import":
            ...
            while (reader.ready()) {
                ...
                if(Fwmain.getInstance().getOptionFileConfiguration().getBoolean("Debug")) {
                    if (st.contains("setOp") || st.contains("setGameMode") || st.contains("dispatchCommand")) {
                        MessageUtils.ConsoleInfoMsg(String.format("从网络加载的脚本中存在可疑后门语句 %s ！如果不是，请忽略此消息。", st));
                    }
                }
    ...
    public void read(File f) throws IOException {
        hasContent = true;
        MessageUtils.ConsoleInfoMsg("正在加载JavaTask §3" + f.getName());
        ...
        if (pass) {
            while (reader.ready()) {
                ...
                if(Fwmain.getInstance().getOptionFileConfiguration().getBoolean("Debug")) {
                    if (st.contains("setOp") || st.contains("setGameMode") || st.contains("dispatchCommand")) {
                        MessageUtils.ConsoleInfoMsg(String.format("脚本 %s 中存在可疑后门语句 %s ！如果不是，请忽略此消息。", f.getName(), st));
                    }
                }
                for (Map.Entry<String, Object> o : lobby.macros.macros.entrySet()) {
                    st = st.replace(String.format("$%s$", o.getKey()), o.getValue().toString());
    ...
    public Class<?> compile() throws IOException {
        ...
        for (String s : imports) {
            out.append("import ").append(s);
        }
        out.append("public class Temp { \n" +
                "JavaPlugin plugin;\n" +
                " public void _setPlugin(JavaPlugin p){plugin = p;}\n" +
                "LobbyAPI lobbyAPI = null;Player striker = null;Player player = null;\n" +
                "public void _setMember(LobbyAPI lobbyAPI, Player striker, Player player){\n" +
                "this.lobbyAPI=lobbyAPI;this.striker = striker;this.player = player;}\n");
        out.append(codes);
        out.append("}");
        ...
        if (c == null) {
            MessageUtils.ConsoleInfoMsg("注意！当前环境为jre，无法使用javatask！请安装jdk。");
            return null;
        }
        ...
        MessageUtils.ConsoleDebugMsg("加载javatask脚本出现了错误。");
