...
public class TaskCompiler {
    ...
    public void getListeners(Set<ListenerTask> func){
        ...
        public Task compile(Lobby lb, File f) {
            fileName = f.getName();
            MessageUtils.ConsoleInfoMsg("正在加载CustomGo脚本 "+f.getName());
            ...
            try {
                ...
                while( (c=(char)r.read()) !=65535){
                    ...
                    switch(c){
                        ...
                        case '\n':
                            if(!v_stack.empty()){
                                MessageUtils.ConsoleErrorMsg("csg脚本"+fileName+" 第"+this.line_counter+"行变量大括号不匹配(缺少})！");
                                v_stack.clear();
                            }
                            if(annotation){
                                annotation = false;
    ...
    Stack<Task> key_com = new Stack<Task>();
        private void analysisCommand(String[] parts,int level){
            ...
            try{
                ...
                MessageUtils.ConsoleInfoMsg("csg脚本"+fileName+" 第"+this.line_counter+"行 出现语法错误！");
                return;
            }
